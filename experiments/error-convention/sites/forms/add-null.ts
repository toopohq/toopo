/**
 * Experiment material, and a verbatim copy of `main:contracts/date/add/reference.ts`. This folder exists only on
 ...
 *
 * date/add@1 as `main` reports it: failure is `null`. This branch's own copy carries the union, so the null arm has to come from the commit that still holds it.
 *
