...
describe('date/add@1 coupling between the two exports', () => {
  it('P7 - a call fails exactly when it has a description', () => {
    ...
    // makes the property decorative or not. It governs every implementation, and one that writes the
    // two independently - which is what optimising the answering path and leaving the diagnostic one
    // alone looks like - can diverge on any input the named cases do not carry. The X-2 probe of the
    // experiment is that implementation, and this property is the only guard in the contract that
    // reddens on it.
    fc.assert(
      fc.property(
        anyDate,
