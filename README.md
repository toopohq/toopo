...
23 mutation batteries inject **844 deliberate defects** into this repository — each one a committed
file naming the exact edit it makes and the verdict it must produce — and re-run the whole suite once
per defect. **802 are caught.**
...
```sh
pnpm install
pnpm run mutation    # tens of minutes; replays all 844 cells and prints the total
