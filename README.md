...
19 mutation batteries inject **689 deliberate defects** into this repository — each one a committed
file naming the exact edit it makes and the verdict it must produce — and re-run the whole suite once
per defect. **656 are caught.** The 33 that survive are each classified, because a survivor count
...
```sh
pnpm install
pnpm run mutation    # tens of minutes; replays all 689 cells and prints the total
