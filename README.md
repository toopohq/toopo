...
23 mutation batteries inject **836 deliberate defects** into this repository — each one a committed
file naming the exact edit it makes and the verdict it must produce — and re-run the whole suite once
per defect. **794 are caught.**
...
```sh
pnpm install
pnpm run mutation    # tens of minutes; replays all 836 cells and prints the total
