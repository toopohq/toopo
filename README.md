...
This repository carries 19 mutation batteries. They inject **649 deliberate defects** into it — each
one a committed file naming the exact edit it makes and the verdict it must produce — and re-run the
whole suite once per defect. **614 are caught.** The 35 that survive are each classified, and the
...
```sh
pnpm install
pnpm run mutation    # about half an hour; replays all 649 cells and prints the total
